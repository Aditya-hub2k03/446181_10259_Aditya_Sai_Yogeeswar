...
public class UserService {
    ...
    public LoginResponseDto login(LoginRequestDto request, HttpSession session) throws ClassNotFoundException {
        try (Connection connection = DBCon.getConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(selectQuery)) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    if (resultSet.next()) {
                        ...
                        if (isPasswordValid) {
                            // Set user session
                            session.setAttribute(USER_ID_SESSION_ATTR, resultSet.getLong("user_id"));
                            session.setAttribute(USER_NAME_SESSION_ATTR, resultSet.getString("user_name"));
                        }
    ...
    public void logout(HttpSession session) {
        session.removeAttribute(USER_ID_SESSION_ATTR);
        session.removeAttribute(USER_NAME_SESSION_ATTR);
        session.invalidate();
    }
    ...
    public Long getCurrentUserId(HttpSession session) {
        return (Long) session.getAttribute(USER_ID_SESSION_ATTR);
    }

    public String getCurrentUserName(HttpSession session) {
        return (String) session.getAttribute(USER_NAME_SESSION_ATTR);
    }
